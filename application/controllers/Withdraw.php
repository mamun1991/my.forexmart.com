    ...
    public function index(){
        ...
        if ($this->session->userdata('logged')) {
            ...
            ")
                ...
                ->prepend_metadata("")
                ->build('withdraw/withdraw_index', $data);
        }else{
            redirect('signout');
        }
    }
    ...
    public function bankTransfer(){
        if($this->session->userdata('logged')) {
            ...
            $this->template->title("ForexMart | Witdrawal Funds | Bank Transfer")
                ...
                ->build('withdraw/bank_transfer',$data);
        }else{
            redirect('signout');
        }
    }
  ...
  public function thunderXPay()
    {
        ...
        if ($this->session->userdata('logged')) {
          ...
          $this->load->library('thunder');
            ...
            $data['minimum_withdraw']=$this->thunder->minimum_limit_per_transaction();
            $data['maximum_withdraw']=$this->thunder->maximum_limit_per_transaction();
            ...
            if ($this->form_validation->run() && !$data['error_msg']) {
                            ...
                            $post_data=array(
                                ...
                                'from_amount'=> $this->input->post('from_amount', true),
                                'fee_balance'=> $this->input->post('fee_balance', true),
                                ...
                                if (in_array($user_id, $test_users)) {
                                 ...
                                 $this->thunder->setAccessType(0);  // for test enviroment
                                    ...
                                    $insertData =array(
                                        ...
                                        'from_amount' => $post_data['from_amount'], 
                                        'fee_balance' => $post_data['fee_balance'],
                                        'withdraw_amount' => $post_data['withdraw_amount'], 
                                    ...
                                    echo "Still waiting for implementation, because the API has not been connected.";
                                    echo "<pre>"; print_r($insertData);
                                    
                                      //$this->general_model->insertmy('thunder_xpay_withdraw', $insertData);
...
//            }
                ...
                $data['metadata_description'] = 'Provide the necessary information to withdrawal via ThunderXPay. Request is processed within 24 hours.';
                ...
                $view = 'withdraw/thunderx_pay';

                $this->template->title('ForexMart | Withdrawal - ThunderXPay')
                    ->set_layout('internal/main')
...
public function getThunderXPayWithdrawFee(){
    if ($this->input->is_ajax_request()) {
       ...
       echo $this->tunder->getFee(amount,"w");
        ...
        if($this->session->userdata('logged')) {
            ...
            $this->template->title("ForexMart | Witdrawal Funds | MegaTransfer")
                ...
                ->prepend_metadata("
                        ...
                        <script src='" . $js . "bootbox.min.js'></script>
                        ")
                ->build('withdraw/megatransfer',$data);
        }else{
            redirect('signout');
        }
    }
    ...
    public function debit_credit_cards(){
        ...
        if ($this->session->userdata('logged')) {
            ...
            $this->template->title("ForexMart | Withdraw - Debit/Credit Cards")
                ...
                ->build('withdraw/choose_debit_credit_cards', $data);
        } else {
            redirect('signout');
        }
    }
...
//            }
            ...
            $this->template->title("ForexMart | Withdraw - Bank Transfer")
                ...
                ->build('withdraw/choose_bank_transfer', $data);
        } else {
            redirect('signout');
        }
    }
        ...
        if($this->session->userdata('logged')) {
          ...
          /*  if(!$this->session->userdata('login_type')){
            ...
            $this->template->title("ForexMart | Witdrawal Funds | Debit/Credit Cards")
                ->set_layout('internal/main')
                ->prepend_metadata("<script src='" . $js . "jquery.autonumeric.js'></script>")
                ->build('withdraw/debit_credit_cards',$data);
        }else{
            redirect('signout');
        }
    }
        ...
        if($this->session->userdata('logged')) {
           ...
           /* if(!$this->session->userdata('login_type')){
            ...
            $this->template->title("ForexMart | Witdrawal Funds | eMerchant")
                ->set_layout('internal/main')
                ->prepend_metadata("<script src='" . $js . "jquery.autonumeric.js'></script>")
                ->build('withdraw/e-merchantpay',$data);
        }else{
            redirect('signout');
        }
    }
        ...
        if($this->session->userdata('logged')) {
          ...
          /*  if(!$this->session->userdata('login_type')){
            ...
            $this->template->title("ForexMart | Witdrawal Funds | Debit/Credit Cards")
                ->set_layout('internal/main')
                ->prepend_metadata("<script src='" . $js . "jquery.autonumeric.js'></script>")
                ->build('withdraw/debit_credit_cards_megatransfer',$data);
        }else{
            redirect('signout');
        }
    }
    ...
    public function withdrawalProcess($withdrawalData){
       ...
       /* if($withdrawalType === 'SKRILL'){
        ...
        if ($requestStatus != 'RET_OK') {
            ...
            $logData = array(
                'account_number' => isset($account_number) ? $account_number : 0,
                'method' => 'WithdrawRealFund',
                'request_data' => json_encode($apiLogs),
...
//        $getWithdrawableRealFund = $WebService2->get_result('Withrawable_RealFund');
    ...
    public function processFundTransfer($request)
    {
        ...
        if($optType == 3){
           ...
           $apiSecurityCode = $this->general_model->showssingle('its_security_code', 'security_code', $securityCode, 'api_security_code');
            ...
            if (!$checkAmountTransferred['error'] && $checkAmountTransferred['account_with_limit']) {
                ...
                $ret = FXAPI::RequestFundTransferFromCode_AC(array('Amount' => $amountRequested, 'Comment' => $generatedTnxNumber, 'IsAccepted' => 1, 'Code' => $apiSecurityCode['api_security_code'], 'IsConfirm' => true));
                $requestStatus = strtoupper($ret['RET']); //status
                $method  = 'RequestFundTransferFromCode_AC';

                if($requestStatus == 'ERROR FETCHING HTTP HEADERS'){

                    $isMethodFailed = 1;
                   
                    $calcRes = $this->calculateFundTransferAmount($calcParams);
                    $receivedAmount = $calcRes['senderAmount']; //amount credited to receiver
               ...
               // $declinedCommentMt4 = 'DPST_TR_' . $sourceAccount;
                ...
                $method  = 'RequestFundTransferFromCode';

                if($requestStatus == 'ERROR FETCHING HTTP HEADERS'){
                    $isMethodFailed = 1;
                   
                    $calcRes = $this->calculateFundTransferAmount($calcParams);
                    $receivedAmount = $calcRes['senderAmount']; //amount to be deducted to sender
                    $rTicket = '00000'; //ticket
                    $sTicket = '00000'; //ticket
                    $emailReceiveAmt = $amountRequested;
                    $emailDeductedAmt = $receivedAmount;
                }else{
            ...
            FXPP::CI()->load->model('Logs_model');
            $logData = array(
                'account_number' => isset($loginAccount) ? $loginAccount : 0,
                'method' => $method,
                'request_data' => json_encode(array('Amount' => $amountRequested, 'Comment' => $generatedTnxNumber, 'IsAccepted' => 0, 'Code' => $apiSecurityCode['api_security_code'], 'IsConfirm' => true,'Login' => $sourceAccount, 'Receiver' => $receiverAccount,'Code2' => $securityCode)),
                'status' => $requestStatus,
                'date' => FXPP::getCurrentDateTime()
            );
            ...
            if($requestStatus == 'ERROR FETCHING HTTP HEADERS'){ 
                ...
                $calcRes = $this->calculateFundTransferAmount($calcParams);
                $receivedAmount = $calcRes['receiverAmount']; //amount credited to receiver
                $rTicket = 00000; //ticket
                $emailReceiveAmt = $receivedAmount;
                $emailDeductedAmt = $amountRequested;
            }else{
            ...
            $logData = array(
                'account_number' => isset($loginAccount) ? $loginAccount : 0,
                'method' => 'SendMoney',
                'request_data' => json_encode(array('Amount' => $amountRequested, 'Comment' => $generatedTnxNumber,'IsAccepted' => 1, 'Login' => $sourceAccount, 'Receiver' => $receiverAccount)),
                'status' => $requestStatus,
                'date' => FXPP::getCurrentDateTime()
            );
        ...
        switch ($requestStatus) {
            ...
            case 'RET_NO_DATA_TO_SHOW':

                $errorMsg = 'Invalid security code, Please generate a new code and try again.';
                $apiError = true;
                ...
                break;
            case 'ERROR FETCHING HTTP HEADERS': //api request timeout
            case 'RET_OK':
                $success = true;
        ...
        if(!$success){
            ...
            if ($isPending) {
                /*   update security code  start  */
                if($requestStatus != 'RET_NO_DATA_TO_SHOW'){
        ...
        $request_data = array(
            ...
            'received_amount' => abs($emailReceiveAmt),
            'security_code' => $securityCode,
    ...
    public function testCalculate($sourceAccount,$receiverAccount,$amountRequested,$optType = 1){
        $calcParams = array(
            'senderAccount' => $sourceAccount,
            'receiverAccount' => $receiverAccount,
            'type' => $optType, 
      ...
      $res =  $this->calculateFundTransferAmount($calcParams);
      var_dump($res);

    }


    public function calculateFundTransferAmount($params){
        switch($params['type']){
            case 1: // partner to client
            case 2: // partner to partner
                  
                  $senderData =  $this->general_model->showssingle2('partnership','reference_num',$params['senderAccount'],'currency');
                  $senderCurrency = $senderData['currency'];
                  $senderAmount = $params['requestedAmount'];
                  if($params['type'] == 2){
                    $receiverData=  $this->general_model->showssingle2('partnership','reference_num',$params['receiverAccount'],'currency');
                    $toCentAmount = $senderAmount;
                    $receiverCurrency = $receiverData['currency'];
                  }else{
                    $receiverData =  $this->general_model->showssingle2('mt_accounts_set','account_number',$params['receiverAccount'],'mt_account_set_id,mt_currency_base');
                    $receiverCurrency = $receiverData['mt_currency_base'];
                    $toCentAmount = ($receiverData['mt_account_set_id'] == 4  || $receiverData['mt_account_set_id'] == 7 ) ? $senderAmount * 100 : $senderAmount;
                  
                  }
                  $toAmountCurrency = ($senderCurrency == $receiverCurrency) ? $toCentAmount : FXPP::getCurrencyRate($toCentAmount,$senderCurrency,$receiverCurrency);
                  $receiverAmount = FXPP::roundno($toAmountCurrency,2);
            ...
            case 3:
                 ...
                 // client to partner
                 $senderData =  $this->general_model->showssingle2('mt_accounts_set','account_number',$params['senderAccount'],'mt_account_set_id,mt_currency_base');
                 $senderCurrency = $senderData['mt_currency_base'];
                 $receiverAmount = $params['requestedAmount'];
               
                 $receiverData =  $this->general_model->showssingle2('partnership','reference_num',$params['receiverAccount'],'currency');
                 $receiverCurrency = $receiverData['currency'];
                 $toCentAmount = ($senderData['mt_account_set_id'] == 4  || $senderData['mt_account_set_id'] == 7 ) ? $receiverAmount * 100 : $receiverAmount;
                
                 
                 $toAmountCurrency = ($senderCurrency == $receiverCurrency) ? $toCentAmount : FXPP::getCurrencyRate($toCentAmount,$receiverCurrency,$senderCurrency);
                 $senderAmount = FXPP::roundno($toAmountCurrency,2);
               ...
               if ($get_security_codes) {
                   if($get_security_codes['status'] > 0) {
                       ...
                       if ($get_security_codes['affiliate_user_id'] == $client_user_id) {
                           ...
                           $message =  'Security code does not match the account.';
                       }
                   }
...
//                    $partner_account_detail = $this->general_model->whereCondition('all_accounts',array('user_id'=>$invite_info['user_id_after_registration'],'account_type'=>'1'));
                    ...
                    $this->template->title("ForextMart | Witdrawal Funds | Paypal")
                        ->set_layout('internal/main')
                        ->prepend_metadata("<script src='" . $js . "jquery.autonumeric.js'></script>")
                        ->build('withdraw/invite_friend',$data);
    ...
    public function chinaUnionPay() {
        if ($this->session->userdata('logged')) {
            ...
            $this->template->title("Withdrawal Option - China Union Pay")
                ->set_layout('internal/main')
                ->prepend_metadata("<script src='" . $js . "jquery.autonumeric.js'></script>")
                ->build('withdraw/china_union_pay', $data);
        } else {
            redirect('signout');
        }
    }
        ...
        if ( IPLoc::for_id_only() || FXPP::isIndonesianCountry() || IPLoc::Office_and_Vpn()) {
            if ($this->session->userdata('logged')) {
                ...
                $this->template->title(lang('dep_tit'))
                    ->set_layout('internal/main')
                    ->prepend_metadata("")
                    ->build('withdraw/local_bank_id', $data);
            } else {
                redirect('signout');
            }
        } else {
...
//                $data['fee_details'] = number_format((float)$datafee_details, 2, '.', '');
        ...
        if ($this->session->userdata('logged')) {
            ...
            $this->template->title("ForexMart | Deposit - Bank transfer in IDR")
                ...
                ->append_metadata_css("
                       <link rel='stylesheet' href='" . $this->template->Css() . "deposit.css'>
                 ")
                ->build('withdraw/choose_bank_transfer_idr', $data);
            } else {
                redirect(FXPP::loc_url('withdraw'));
            }
        } else {
        ...
        if ($this->session->userdata('logged')) {
            ...
            $this->template->title("ForexMart | Deposit - Bank transfer in MYR")
                ...
                ->append_metadata_css("
                       <link rel='stylesheet' href='" . $this->template->Css() . "deposit.css'>
                 ")
                ->build('withdraw/choose_bank_transfer_myr', $data);
            } else {
                redirect(FXPP::loc_url('withdraw'));
            }
        } else {
        ...
        if ($this->session->userdata('logged')) {
            ...
            $this->template->title("ForexMart | Deposit - Bank transfer in THB")
                ...
                ->append_metadata_css("
                       <link rel='stylesheet' href='" . $this->template->Css() . "deposit.css'>
                 ")
                ->build('withdraw/choose_bank_transfer_thb', $data);
            } else {
                redirect(FXPP::loc_url('withdraw'));
            }
        } else {
            ...
            if(IPLoc::Office() || FXPP::isVietnamCountry()) {
                ...
                $this->template->title("ForexMart | Withdraw - Bank transfer in VND")
                    ...
                    ->append_metadata_css("
                       <link rel='stylesheet' href='" . $this->template->Css() . "deposit.css'>
                 ")
                    ->build('withdraw/choose_bank_transfer_vnd', $data);
            } else {
                redirect(FXPP::loc_url('withdraw'));
            }
    ...
    public function zotapay_card(){
        ...
        if(!FXPP::isPaymentMethodAvailable('ZOTAPAY_CARD')){
           redirect('withdraw/');
        }
        ...
        if($this->session->userdata('logged')) {
            ...
            $this->template->title("ForexMart | Witdrawal Funds | Debit/Credit Cards")
                ->set_layout('internal/main')
                ->prepend_metadata("<script src='" . $js . "jquery.autonumeric.js'></script>")
                ->build('withdraw/debit_credit_cards_zotapay',$data);
        }else{
            redirect('signout');
        }
    }
    ...
    public function nova2pay(){
        ...
        if($this->session->userdata('logged')) {
            ...
            if(!FXPP::isPaymentMethodAvailable('NOVA2PAY')){
                redirect('withdraw/');
            }
            ...
            $this->template->title("ForexMart | Witdrawal Funds | Debit/Credit Cards")
                ->set_layout('internal/main')
                ->prepend_metadata("<script src='" . $js . "jquery.autonumeric.js'></script>")
                ->build('withdraw/debit_credit_cards_nova2pay',$data);
        }else{
            redirect('signout');
        }
    }
