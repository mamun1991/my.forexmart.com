...
//        }
    ...
    public function getSecurityCode() {
        ...
        do {
            ...
            $security = $this->general_model->showssingle2($table = 'its_security_code', $field = 'security_code', $id = $security_code, $select = 'security_code', $order_by = '');
        } while ($security);
...
//        }
        ...
        if (!$code_details){
            ...
            $ret = FXAPI::GenerateCodeForInternalTransfer(array());
            $apiCode = $ret['data']->Common;
...
//                    $data['ref_table'] =  $this->partners_model->getReferralActivtiesDetailsV2($user_id);
//                }else{
//
...
//                                $TotalBonusFund = $WebService2->get_result('TotalBonusFund');
    ...
    public function referrals_export2(){

            $dateFrom = FXPP::unixTime(date("Y-m-d 0:0:0",strtotime("2015-05-05")));
            $dateTo = FXPP::unixTime(date("Y-m-d 23:59:59"));
            $offset = 0;
            ...
            if ($dataCount > 0) {
                ...
                $header = FXPP::referrals_table_permission(FXPP::referrals_table(),$this->ref_table_permission());
                $tableView = "<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:x='urn:schemas-microsoft-com:office:excel'><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]-></head><body><table  >";
                $tableView .= '<thead><tr>';
                foreach ($header as $v) {
                ...
                foreach ($referralList as $key => $obj) {
                    ...
                    $ver_stat = ($accountStatus[$accountNumber]['status'] == 1 || $accountStatus[$accountNumber]['status'] == 3) ? '<span style="color:green;">Verified</span>' : '<span style="color:red;">Non-Verified</span>';
                    ...
                    $regDate  = $unixRegDate->format('Y-m-d');

                    $accountNumberTd =  $accountNumber . " (" . $this->getGroupCodeType($accountGroup) . " )" ; //"Account number of referral",
...
//            }
        ...
        if ($dataCount > 0) {
            ...
            if(IPLoc::IPOnlyForTq()) {
              ...
              //  print_r ($accounts); exit();
            //  print_r($saldoList); exit();   // all 4 data saldo
            }
            ...
            foreach ($referralList as $key => $obj) {
                ...
                $accountNumberTd =   '<span data-level ="1" ref="'.$accountNumber.'" class="btn-expand"><i  class="fa fa-plus"></i></span> ' .  $accountNumber . " (" . $this->getGroupCodeType($accountGroup) . ') <i data-html="true" rel="tooltip" class="fa fa-question-circle holder-img " data-toggle="tooltip" data-placement="right" title="" data-original-title="<p><b>Acc type legend:</b><br> Cl - Classic<br> Pr - Pro<br> Cn - Cents<br> Zr - Zero Spread<br> St - Standard<br> Pa - Partner</p>" aria-describedby="tooltip96414"></i>'; //"Account number of referral",


               // re-arrange table FXPP-12613

                $logintype =  $this->general_model->showssingle('users', 'id', $userID, '*')['login_type'];
                $agentCode =  $logintype==0?$this->general_model->showssingle('users_affiliate_code', 'users_id', $userID, 'affiliate_code')['affiliate_code']:$this->general_model->showssingle('partnership_affiliate_code', 'partner_id', $userID, 'affiliate_code')['affiliate_code'];
...
//                    $full_table = array(
//                        1 => 'acc no',
//                        2 => 'name', //"Referral's name",
//                        0 => 'date', //"Date of registration",
//                        6 => $Email, //"Email",
//                        7 => $PhoneNumber, // "Phone number",
//                        3 => 'balance', //"Balance",
//                        4 => 'comission', // "Total amount of commission",
//                        5 => 'lots', //"Number of Lots",
//                        11 => 'saldo', //"Number of Lots",  //
//                        8 => $ver_stat // "Verified Status",
//                    );
                    ...
                    $full_table = array(
                        ...
                        11 => $saldo, //"Number of Lots",
                        8 => $ver_stat, // "Verified Status",
                        12 => $agentCode //"Agent Affiliate Code"
                    );
    ...
    public function getReferralsByLevel(){
        ...
        foreach ($referralList as $key => $obj) {
            ...
            $verStatus = ($obj['accountstatus'] == 1 || $obj['accountstatus'] == 3) ? '<span style="color:green;">Verified</span>' : '<span style="color:red;">Non-Verified</span>';
   ...
   public function getAccountStatus($accountNumbers = array()){
    ...
    public function getReferralTotalLots($requestData = array(),$isTotal = false,$groupData = array()){
        ...
        $accountTypes = $this->getAccountType($requestData['Accounts'])['accounts']; // get account status
       /* if(IPLoc::IPOnlyForVenus()){
            echo '<pre>';
           print_r($referralLotsData);exit();
        }*/
         
       foreach($referralLotsData as $k => $v){
            // if(IPLoc::IPOnlyForVenus()){
        //    var_dump($totalLots);exit();
        // }
        
         // $group =  $this->getGroupCodeType($groupData[$v->key]);
          $group =  $accountTypes[$v->key]['account_type'];
        
            $lots =  $v->value;
            if($group == '7' || $group == '4'){ //cents
                $lots /= 100;
...
//       print_r($res);
    ...
    public function requestCommission(){
        ...
        if ($this->session->userdata('logged')) {
            ...
            $dateFrom = isset($dateFrom) ? FXPP::unixTime($dateFrom) : FXPP::unixTime(date('Y-m-d 23:59:59', strtotime('- 1 day')));
          ...
          $dateTo = isset($dateTo) ? FXPP::unixTime($dateTo) : FXPP::unixTime(date('Y-m-d 23:59:59', strtotime('now')));
            ...
            $requestRes = $WSV->GetCommissionHistory($requestData)['Data'];

            $totalCommission = isset($requestRes->TotalFromRange) ? $requestRes->TotalFromRange : 0;
            $dataCount = $requestRes->DataCount;
            $tradesData = $requestRes->Transactions;
            $pendingList = array();
            if ($dataCount > 0) {
                foreach ($tradesData->TradeRecord as $key => $value) {
                    $tempArray = array(
                        'DT_RowId' => $ctr,
                        $ctr,
                        gmdate("Y-m-d H:i:s", $value->CloseTime),
    ...
    public function getAccountTotalSaldo($accountNumbers = array()){
        ...
        $saldoTransferData = $this->deposit_model->getTransitTransferByDate($accountNumbers,$from, $to);

        if(IPLoc::IPOnlyForTq()){
          //  print_r($saldoData); exit();
             // echo $this->db->last_query(); exit();
        }
        ...
        foreach($saldoTransferData as $k1 => $v1){
            $accounts[$v1['account_number']]['saldo']  += $v1['saldo_amount'];
    ...
    public function getAccountType($accountNumbers = array()){
        $accountData =  $this->account_model->getListAccountStatus($accountNumbers);
        $accounts = array();
