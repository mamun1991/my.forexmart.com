...
<style type="text/css">
    ...
    .th-lev, .th-cur , .th-bal , .th-status, .type-td{
        width: 10%;
    }
    .th-type{
        width:15%;
...
<div class="col-md-12 col-sm-9 btn-trading-demo-holder">
...
<table id="first-accounts-tables" class="table table-striped tab-my-acct arabic-part-table arabic-trans-history-table head-th" style="margin-bottom:0px;">
    <thead>
    <tr>
        <th class="th-accnum"><?= lang('macv_00'); ?></th>
        <th class="th-lev"><?= lang('macv_01'); ?></th>
        <th class="th-cur"><?= lang('macv_02'); ?></th>
        <!-- <th>Free Margin</th>-->
        <th class="th-bal"><?= lang('macv_03'); ?></th>
        <th class="th-status"><?= lang('macv_04'); ?></th>
        <th class="th-type"><?= lang('macv_05'); ?></th>
        <th class="th-swapf"><?= lang('macv_06'); ?></th>
        <th class="th-action"><?= lang('mya_33'); ?></th>
    </tr>
    </thead>
</table>
<div style="max-height:500px;overflow-y:scroll;min-height:115px;">
<table id="tb_dshbrd" class="table table-striped tab-my-acct arabic-part-table arabic-trans-history-table" style="margin-bottom:0px;">
<tbody id="accounts-table">
    ...
    <tr>
        ...
        <td class="sec-td"><span id="txt_leverage"><?php echo $account['leverage'] ?></span>
            <?php
            if($show_fx_bonus['is_showfxbonus']==1 ){
                ...
                <select id="select_leverage" class="form-control round-0"
                        style="display: none;"><?php echo $leverage ?></select>
                <?php } ?>
        </td>
        <td class="cur-td"><?php echo $account['mt_currency_base'] == 'RUB' ? 'RUR' : $account['mt_currency_base']; ?></td>
        <!-- <td><?php /*echo $account['amount'] == '' ? 0 : $account['amount']; */
            ?> </td>-->
        <td class="balance bal-td"><?php echo $balance == '' ? 0 : $balance; ?></td>
        <td class="type-td" id="live-client-status skt <?php echo $account['mt_status']; ?>"><?php echo ((!isset($account['mt_status']) || trim($account['mt_status']) === '')) ? lang('mya_26') : lang('mya_27'); ?></td>
        <?//=lang('mya_27'); //"Verified" ?> <?//=lang('mya_26'); //"Read only" ?> <!-- <td><?php /*echo $account['mt_type'] ? 'Trading' : 'Demo' */ ?></td>-->
        <?php
        if(IPLoc::Office()) { echo $account['account_type'];   }
        //FXPP-4852
        ...
        ?>
        <td style="font-size: 12px;"><?php /*if (IPLoc::Office()) { echo $account_type;   } else {*/ echo $account['account_type']; // } ?></td>
        <td  class="swapf-td"><input type="checkbox" id="chk_swap"<?php echo $account['swap_free'] ? ' checked' : '' ?><?= isset($reg_loc) && ($reg_loc == 7) ? 'disabled' : ''; ?> /></td>
    ...
    if(IPLoc::Office()){
        if (count($accounts2) > 0) {
            ...
            <tr>
                ...
                <td id="live-client-status1" style="<?=$color;?>"><?=$stat_inactive;?></td>
...
//                    }
                ...
                <td><input type="checkbox" id="chk_swap"<?php echo $account['swap_free'] ? ' checked' : '' ?> disabled/></td>
                <td>
...
<table id="tb_dshbrd2" class="table table-striped tab-my-acct">
    ...
    <tbody id="partner-accounts-table">
        <?php if( count($accounts) > 0 ){
        foreach( $accounts as $account ){ ?>
        <tr>
            <td>
            ...
            <td>
                ...
                echo ($p_status['accountstatus']==1 or $p_status['accountstatus']==3) ? "Verified" : "Read Only";
            }?>
            </td>
        </tr>
            <?php
...
//                $('.edit-swap-text').html('You are about to turn off the Swap-Free field.');
        ...
        jQuery('#cancel_swap').on('click', function() {
