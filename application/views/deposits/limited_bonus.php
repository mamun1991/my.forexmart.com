...
$(document).ready(function () {
    $(".ckbox_class").after('<img src="<?= $this->template->Images()?>100-percent-bonus.png" class="img-responsive"/>');
    $(".ckbox_class1").after('<img src="<?= $this->template->Images()?>50-percent-bonus.png" class="img-responsive"/>');
});
...
$(document).on("click", '.ckbox_class', function () {
    ...
    if ($(this).is(':checked')) {
        ...
        if (val > 2000) {
            $(this).after('<img src="<?= $this->template->Images()?>50-percent-bonus.png" class="img-responsive"/>');
        } else {
            $(this).after('<img src="<?= $this->template->Images()?>100-percent-bonus.png" class="img-responsive"/>');
        }
        ...
        if (val > 2000) {
            $(this).after('<img src="<?= $this->template->Images()?>50-percent-bonus-inactive.png" class="img-responsive"/>');
        } else {
            $(this).after('<img src="<?= $this->template->Images()?>100-percent-bonus-inactive.png" class="img-responsive"/>');
        }
...
$(document).on("click", '.ckbox_class1', function () {
...
$(document).ready(function () {
    ...
    var id = $('#select-deposit-logo').val();
    $(".imgclslogo").hide();
    $("#" + id).show();
    ...
    $("#set_amount").val("");
    $("#bonus_deposit_amount").val("");
    $("#amount").val("");
    // $('.finance-dep-table table input[type=radio] ').removeAttr('checked');
...
$(document).on('change', '#select-deposit-logo', function () {
    ...
    switch (id) {
        case 'idr':
            str = "Sekarang anda bisa mendepositokan dana ke akun ForexMart melalui bank lokal dengan mudah. Kami menerima transfer bank lokal dari seluruh bank terbesar di Indonesia.";
            break;
        case 'myr':
            str = "It is now easy to deposit funds to your ForexMart account through local bank. We accept local bank transfers from all biggest banks in Malaysia.";
            break;
        case 'bt':
            str = "Transfer money from your bank account to ours directly. As you deposit in your local currency, the receiving bank will convert the funds to your preferred currency. ";
            break;
        case 'ddcc':
            str = "Entrust your funds with one of the world's leading online payments. Deposit money to your tradingaccount using your Visa credit/debit card. ";
            break;
        case 'skl':
            str = "Skrill (Moneybookers) enables you to transfer funds to your trading account and receive money via email. ";
            break;
        case 'ntl':
            str = "Being an independent money transfer business, Neteller offers a quick, easy method of depositing or withdrawing into your trading account. ";
            break;
        case 'pxm':
            str = "Paxum offers a fast, efficient, and affordable payment solution to deposit or withdraw money with ease. ";
            break;
        case 'ppl':
            str = "Paypal is the faster, safer way to send money, make an online payment, receive money or set up a merchant account.";
            break;
        case 'web':
            str = "WebMoney is a global e-wallet providing an avenue for making deposits or withdrawals into your trading account. ";
            break;
        case 'pco':
            str = "PayCo, having the lowest transaction fee worldwide, provides the most flexible, safest way of transferring and receiving money online, and paying bills. ";
            break;
        case 'qwi':
            str = "Buy online and offline, pay bills and translate money to close at any time.";
            break;
        case 'mgt':
            str = "MegaTransfer is a worldwide, online payment that is efficient, convenient, and innovative. ";
            break;
        case 'btc':
            str = "Bitcoin is an innovative payment network and a new kind of money. Bitcoin uses peer-to-peer technology to operate with no central authority or banks. ";
            break;
        case 'ynx':
            str = "More than just a wallet. Yandex.Money is a simple service providing online payments. Pay for home utilities, transfer money to your relatives, or buy goods at online stores. ";
            break;
        case 'mtr':
            str = "MONETA.Assistant interface is an application for processing online payments via various payment methods. ";
            break;
        case 'sft':
            str = "Easy, comfortable and secure - without registration. Automatic data transfer and the real-time transaction notification enable a smooth payment process and a faster delivery.";
            break;
        case 'hwt':
            str = "";
            break;
...
// check is bouns active 
    ...
    if (val == 'myr') {
        // $(location).attr('href','
        <?=FXPP::loc_url('deposit/bank-transfer-myr');?>');
...
<div class="finance-image-rep col-lg-4 col-md-4 col-sm-4 col-xs-12">
    <img id="idr" src="<?= $this->template->Images() ?>/indcard.png" class="img-responsive imgclslogo imgclsdtl"/>
    ...
    <img id="ddcc" src="<?= $this->template->Images() ?>/ccard.png" class="img-responsive imgclslogo"/>
...
<div class="finance-description col-lg-4 col-md-4 col-sm-4 col-xs-12">
    <p id='desc'>Sekarang anda bisa mendepositokan dana ke akun ForexMart melalui bank lokal dengan mudah. Kami menerima
        transfer bank lokal dari seluruh bank terbesar di Indonesia.</p>
</div>
</div>

<div class="hidden-finance-dep-local paymentforidr">
    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-2 local-bank-content">
        <!-- <img src="<?= $this->template->Images() ?>/50-percent-bonus.png"/> -->
        <img src="<?= $this->template->Images() ?>/100-percent-bonus.png"/>
    </div>
    <div class="col-lg-10 col-md-10 col-sm-10 col-xs-10 local-bank-content">
        <p>Have the opportunity to get 50% or 100% of the total amount of money deposited to your account.</p>
    </div>
...
<div class="finance-deposit-second-container fdsc">
    <div class="finance-dep-table-parent col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="finance-dep-table">
            <table id="financetable">
                ...
                <tbody>
                <tr>
                    <td><input value="10000" name="bonus_deposit_amount" rel="fpb" type="radio" class="radio_class"/>
                    </td>
                    <td>$ 10,000</td>
                    <td>
                        <div class="checking-bonus">
                            <input type="checkbox" class="ckbox_class1" checked/>

                        </div>
                    </td>
                    <td>$ <span class="get">15,000</span></td>
                </tr>
                <tr>
                    <td><input value="5000" name="bonus_deposit_amount" rel="fpb" type="radio" class="radio_class"/>
                    </td>
                    <td>$ 5,000</td>
                    <td>
                        <div class="checking-bonus">
                ...
                <tr>
                    <td><input value="3000" name="bonus_deposit_amount" rel="fpb" type="radio" class="radio_class"/>
                    </td>
                    <td>$ 3,000</td>
                    <td>
                        <div class="checking-bonus">
                ...
                <tr>
                    <td><input value="2000" name="bonus_deposit_amount" rel="hpb" type="radio" class="radio_class"/>
                    </td>
                    <td>$ 2,000</td>
                    <td>
                        <div class="checking-bonus">
    ...
    <div class="finance-total-amount col-lg-4 col-md-4 col-sm-4 col-xs-12">
        <div class="finance-total-sub-amount">
            ...
            <div class="finance-total-amount-right-child" id="img_show_percent">
                <img id="img_show_percent_img" src="<?= $this->template->Images() ?>/100-percent-bonus.png"
                     class="img-responsive img-other">
            </div>
        </div>
    </div>
    <div class="finance-total-amount col-lg-4 col-md-4 col-sm-4 col-xs-12">
    ...
    <div class="regulatory-note col-lg-12 col-md-12 col-xs-12 col-sm-12">
        <a href="javascript:;">All deposits are being made to EU-regulated and licensed company Tradomart Ltd, CySEC
            license number 266/15</a>
    </div>
</div>
