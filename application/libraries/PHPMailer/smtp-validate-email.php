...
class SMTP_Validate_Email {
    ...
    private $from_user = 'noreply@mail.forexmart.com';
    // default host of sender
    private $from_domain = 'mail.forexmart.com';
    // the host we're currently connected to
    private $host = 'smtp.mail.forexmart.com';
...
// test mail domain validation ========================================================================================>>>>>>>>>>>>>>>>>>> 
 public function validate_test($emails = array(), $sender = '') {
        $this->results = array();
        if (!empty($emails)) {
            $this->set_emails($emails);
        }
        ...
        foreach ($this->domains as $domain => $users) {
            ...
            if ($this->connected()) {
                try {
                    ...
                    if ($this->helo()) {
                        // try issuing MAIL FROM
                        if (!($this->mail_test($this->from_user . '@' . $this->from_domain))) {
                            // MAIL FROM not accepted, we can't talk
                            $this->set_domain_results($users, $domain, $this->no_comm_is_valid);
                        }
                        ...
                        */
                        if ($this->connected()) {
                            ...
                            if ($this->connected()) {
								...
								echo "<br>";
								print_r($this->rcpt_test($users[0].'@' . $domain));
								exit;
								...
								*/
                                ...
                                foreach ($users as $user) {
                                    $address = $user . '@' . $domain;
                                    $this->results[$address] = $this->rcpt_test($address);
                                    $this->noop();
                                }
                            }
                            // saying buh-bye if we're still connected, cause we're done here
    ...
    protected function mail_test($from) {
        if (!$this->state['helo']) {
            throw new SMTP_Validate_Email_Exception_No_Helo('Need HELO before MAIL FROM');
        }
        // issue MAIL FROM, 5 minute timeout
        $this->send('MAIL FROM:<' . $from . '>');
        try {
            $this->expect_test(self::SMTP_GENERIC_SUCCESS, $this->command_timeouts['mail'],false,true);
            // set state flags
            $this->state['mail'] = true;
            $this->state['rcpt'] = false;
            return true;
 ...
 protected function rcpt_test($to) {
        // need to have issued MAIL FROM first
        if (!$this->state['mail']) {
            throw new SMTP_Validate_Email_Exception_No_Mail_From('Need MAIL FROM before RCPT TO');
        }
        ...
        try {
            ...
            // process the response
            try {
                $this->expect_test($expected_codes, $this->command_timeouts['rcpt'],false,true);
                $this->state['rcpt'] = true;
                $is_valid = true;
            } catch (SMTP_Validate_Email_Exception_Unexpected_Response $e) {
                $this->debug('Unexpected response to RCPT TO: ' . $e->getMessage());
...
protected function expect_test($codes, $timeout = null, $empty_response_allowed = false,$edata=false) {
        if (!is_array($codes)) {
            $codes = (array) $codes;
        }
        $code = null;
        $text = '';
        try {
            $text = $line = $this->recv_test($timeout,true);
			
			
			
			if($edata){		
					echo "<pre>";print_r($text); echo "====================____=====================>"; echo "<br>"; exit;
			}
...
protected function recv_test($timeout = null,$edata=false) {
		...
		if($edata){		
					echo "<pre>";print_r((fgets($this->socket, 1024))); echo "====================____=====================>"; echo "<br>"; exit;
			}
		...
		echo "<<<<----------------------------->>>>>";
		echo "<br>";
		echo print_r(fgets($this->socket, 1024));
		echo "<br>";
		echo "________________________________________>";	
		echo "<br>";
		print_r($this->debug('<<<recv: ' . $line));
		exit;
