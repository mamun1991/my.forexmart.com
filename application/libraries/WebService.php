...
Class WebService {
    ...
    protected $fm_service_id = '1001';
    protected $saldo_password = 'JGW2aS8pNKfYnmhu';
    protected $qoutes_password = 'Buev7z74CwbA2MQu';
    protected $report_password = 'A79sWvGu88YpBC7u';
...
//        'converter' => 'http://136.243.89.90:9088/Converter.svc?singleWsdl'
       ...
       // 'converter' => 'http://136.243.105.122/CurrencyConverter.svc?singleWsdl',
       'converter' => 'http://qsvc.forexmart.com/CurrencyConverter.svc?singleWsdl',
...
//        'bitcoin' => 'http://78.47.95.22:8055/BitcoinPaymentService.svc?singleWsdl',
        ...
        //'qoutes_ticks' => 'http://136.243.105.122:8080/QuotesAndTicks.svc?singleWsdl',
        'qoutes_ticks' => 'https://qsvc.forexmart.com:4434/QuotesAndTicks.svc?singleWsdl',
        'copytrader' => 'http://136.243.89.90:8000/ForexCopy.svc?singleWsdl',
    );
...
//                    'Leverage' => $account_info['leverage'],
    ...
    public function open_GetAccountActiveTrades( $account_info = array() ){
        ...
        try {
            ...
            $oAccountData = $this->proxy->GetAccountActiveTrade($data);
            $this->request_status = $oAccountData->GetAccountActiveTradeResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetAccountActiveTradeResult);
            return true;
        } catch (SoapFault $e)  {
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetAccountActiveTrade',$eData)
            );
        }
    }
    /**
     ...
     * GetAccountTradesHistory both have demo and live method
     */
    public function open_GetReferralsTradeHistory( $account_info = array() ){

        $eData = array();
        try {
            $data = array(
                'partner' => $account_info['iLogin'], // int partner account Number
                'from' => $account_info['from'], // DateTime::createFromFormat  format('Y-m-d\TH:i:s')
                'to' => $account_info['to'], // DateTime::createFromFormat  format('Y-m-d\TH:i:s')
                'isHistory'=>1,
                'serviceId' =>  $this->service_id, //int
            ...
            $oAccountData = $this->proxy->GetReferralsTrade($data);    
               
            $this->request_status = $oAccountData->GetReferralsTradeResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetReferralsTradeResult);
            return true;
        } catch (SoapFault $e)  {
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'TradeDataResult',$eData)
            );
        }
    }
...
//            $this->request_status = $oAccountData->GetServerTimeResult->ReqResult;
     ...
     * RequestAccountFinanceRecordsByDate both and live method
     */
    public function open_RequestAccountFinanceRecordsByDate_groupBy( $account_info = array() ){
    ...
    private function combineFeeWithDepositTxn(){

        $deposit_txn = array();
        $fee_txn = array();
        $other_txn = array();
      
        foreach($this->result['FinanceRecords']->FinanceRecordData as $d){
        ...
        foreach($deposit_txn as $d){
            ...
            foreach($fee_txn as $k){
                if(strpos($k->Comment,$txn_id) !== false){
                    $d->Amount = number_format( ($d->Amount + $k->Amount),4);
                }
            }
        }

        $txn = array_merge($deposit_txn,$other_txn); // Fix task FXPP-11524
        //$txn = ($deposit_txn + $other_txn);
        $this->result['FinanceRecords']->FinanceRecordData = $txn;
    }
...
//            FXPP::print_data($data);exit;
 ...
 public function GetAccountCancelledPendingOrders( $account_info = array() ){
    ...
    /**
     * DeactivateAccountTrading used for Account Verification to deactivate trading Admin side
     *
     */

    public function open_DeactivateAccountTrading( $account_info = array() ){
        $eData = array();
        try {
            $data = array(
                'iLogin' => $account_info['AccountNumber'],
 ...
 */
    ...
    public function depositorInformation2($accountNumber){
        ...
        if(empty($resultcheck)) {
            ...
            if($exhousr=="24" or empty($finalCheckN))
            {
                        ...
                        $subject = "Active Malaysian Client Report";
                        $edatas['email_data']['heading']='Active Malaysian Client Report.';
                        $ci->general_model->sendEmailVisitor('default_mail', $subject,'jayhens.snow@gmail.com', $edatas, $config,null);
    ...
    public function open_UpdateTradersCondition($account_info = array()){
        ...
        try {
          ...
          //  print_r($data);
            ...
            if(!empty($account_info['conditions_values_5'])){
                $data['condtion'][2]['Key'] = 5;
                $data['condtion'][2]['Value'] = $account_info['conditions_values_5'];
            }
//            if(!empty($account_info['conditions_values_6'])){
//                $data['condtion'][2]['Key'] = 5;
//                $data['condtion'][2]['Value'] = $account_info['conditions_values_6'];
//            }
    ...
    public function Open_GetMonthlyLots($account_info = array()){
        $eData = array();
        try {
            $data = array(
                'iLogin'    => $account_info['iLogin'],
            ...
            $oAccountData = $this->proxy->GetMonthlyLots($data);
            $this->request_status = $oAccountData->GetMonthlyLotsResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetMonthlyLotsResult);
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetMonthlyLots',$eData)
            );
        }
    }


    public function get_symbol_last_timestamp( $arg = array() ){
        $eData = array();
        try {
            $data = array(
                'strSymbol' =>  $arg['symbol'],
            );
            $oAccountData = $this->proxy->GetSymbolLatestQuoteData($data);
            $this->request_status = $oAccountData->GetSymbolLatestQuoteDataResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetSymbolLatestQuoteDataResult);
            return true;
        } catch (SoapFault $e)  {
            return array(
                'SOAPError' => true,
    ...
    public function RequestLastQoutes($account_info) {
        $eData = array();

        $data = array(
            'request' => array(
                'From'        => $account_info['From'], // unix time
                'Id'          => $this->fm_service_id,
                'Password'    => $this->qoutes_password,
                'Symbols'     => $account_info['Symbols'], //no requested symbol means return all current qoutes
                'TimeFrame'   => $account_info['TimeFrame'], //default 0
                'To'          => $account_info['To'],  // unix time
            )
        );

        try {
            $oAccountData = $this->proxy->RequestLastQuotes($data);
            $this->request_status = $oAccountData->RequestLastQuotesResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->RequestLastQuotesResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'RequestLastQoutes',$eData)
            );
        }
    }


    public function GetAllCopyTrader($account_info) {
        $eData = array();

        $data = array(
            'req' => array(
                'Offset'      => $account_info['offset'],
                'Limit'       => $account_info['limit'], //prefer max 20
                'Order'       => 'PROFIT_DESC', //PROFIT or  REGISTER_DATE
                'Id'          => $this->fm_service_id,
                'Password'    => 'eJR5RdZfA86s4m78',
                'Session'     => 'abc123456',
                'UserId'      => $account_info['accountNumber'],  //visitor account number
            )
        );

      
        try {
            //$functions = $this->proxy->__getFunctions ();
            // $types= $this->proxy->__getTypes ();
            $oAccountData = $this->proxy->GetAllTrader($data);
            $this->request_status = $oAccountData->GetAllTraderResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetAllTraderResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetAllTrader',$eData)
            );
        }
    }

    public function SearchTrader($account_info) {
        $eData = array();
        ...
        $data = array(
            'req' => array(
                ...
                'Order'       => 'PROFIT_DESC', //PROFIT or  REGISTER_DATE
                'Id'          => $this->fm_service_id,
                'SearchString'  => $account_info['search'],
                'Password'    => 'eJR5RdZfA86s4m78',
                'Session'     => 'abc123456',
                'Trader'      => 0,
                'UserId'      => $account_info['accountNumber'],  //visitor account number
        ...
        try {
            ...
            $oAccountData = $this->proxy->SearchTrader($data);
            $this->request_status = $oAccountData->SearchTraderResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->SearchTraderResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'SearchTrader',$eData)
            );
        }
    }

    public function GetTraderCopyTerms($account_info) {
        $eData = array();
        ...
        $data = array(
            'req' => array(
                ...
                'Password'    => 'eJR5RdZfA86s4m78',
                'Session'     => 'abc123456',
                'Trader'     => $account_info['traderAccount'],
                'UserId'      => $account_info['visitor'],  //visitor account number
            )
        );


        try {
            $oAccountData = $this->proxy->GetTraderCopyTerms($data);
            $this->request_status = $oAccountData->GetTraderCopyTermsResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetTraderCopyTermsResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetTraderCopyTerms',$eData)
            );
        }
    }
    public function GetUserBalanceCache($account_info) {
        $eData = array();
        ...
        try {
            $oAccountData = $this->proxy->GetUserBalanceCache($data);
            $this->request_status = $oAccountData->GetUserBalanceCacheResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetUserBalanceCacheResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetUserBalanceCache',$eData)
            );
        }
    }
    public function GetTraderUserDetails($account_info) {
        $eData = array();
        ...
        try {
            $oAccountData = $this->proxy->GetTraderUserDetails($data);
            $this->request_status = $oAccountData->GetTraderUserDetailsResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetTraderUserDetailsResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetTraderUserDetails',$eData)
            );
        }
    }
    public function GetTraderDescription($account_info) {
        $eData = array();
        ...
        try {
            $oAccountData = $this->proxy->GetTraderDescription($data);
            $this->request_status = $oAccountData->GetTraderDescriptionResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetTraderDescriptionResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetTraderDescription',$eData)
            );
        }
    }
    public function GetBalanceHistory($account_info) {
        $eData = array();
        ...
        try {
            $oAccountData = $this->proxy->GetBalanceHistory($data);
            $this->request_status = $oAccountData->GetBalanceHistoryResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetBalanceHistoryResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetBalanceHistory',$eData)
            );
        }
    }
    public function GetProfitHistory($account_info) {
        $eData = array();
    ...
    public function GetConnectionsCopyTerms($account_info) {
        $eData = array();
        ...
        try {
            $oAccountData = $this->proxy->GetConnectionsCopyTerms($data);
            $this->request_status = $oAccountData->GetConnectionsCopyTermsResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetConnectionsCopyTermsResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetConnectionsCopyTerms',$eData)
            );
        }
    }
    public function GetCloseTradesFromConnection($account_info) {
        $eData = array();

        $data = array(
            'req' => array(
                'ConnectionId' => $account_info['ConnectionId'],
                'Offset'      => $account_info['Offset'], //prefer max 20
                'Limit'       => $account_info['Limit'], //prefer max 20
                'Id'          => $this->fm_service_id,
                'Password'    => 'eJR5RdZfA86s4m78',
                'Session'     => 'abc123456',
        ...
        try {
            $oAccountData = $this->proxy->GetCloseTradesFromConnection($data);
            $this->request_status = $oAccountData->GetCloseTradesFromConnectionResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetCloseTradesFromConnectionResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetCloseTradesFromConnection',$eData)
            );
        }
    }
    public function GetOpenTradesFromConnection($account_info) {
        $eData = array();
        ...
        try {
            $oAccountData = $this->proxy->GetOpenTradesFromConnection($data);
            $this->request_status = $oAccountData->GetOpenTradesFromConnectionResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetOpenTradesFromConnectionResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetOpenTradesFromConnection',$eData)
            );
        }
    }
    public function GetRollOverProfit($account_info) {
        $eData = array();
        ...
        try {
            $oAccountData = $this->proxy->GetRollOverProfit($data);
            $this->request_status = $oAccountData->GetRollOverProfitResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetRollOverProfitResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetRollOverProfit',$eData)
            );
        }
    }
    public function GetAccountsBalance($account_info) {
        $eData = array();
        ...
        try {
            $oAccountData = $this->proxy->GetAccountsBalance($data);
            $this->request_status = $oAccountData->GetAccountsBalanceResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetAccountsBalanceResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetAccountsBalance',$eData)
            );
        }
    }
    public function GetMyTrader($account_info) {
        $eData = array();
        ...
        $data = array(
            'req' => array(
                ...
                'Session'     => 'abc123456',
                'Trader'     => 0,
                'UserId'      => $account_info['AccountNumber'],
            )
        );

        try {
            $oAccountData = $this->proxy->GetMyTrader($data);
            $this->request_status = $oAccountData->GetMyTraderResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetMyTraderResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetMyTrader',$eData)
            );
        }
    }
    public function GetMyFollower($account_info) {
        $eData = array();
        ...
        try {
            $oAccountData = $this->proxy->GetMyFollower($data);
            $this->request_status = $oAccountData->GetMyFollowerResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetMyFollowerResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetMyFollower',$eData)
            );
        }
    }
    public function GetPastSubscription($account_info) {
        $eData = array();
        ...
        try {
            $oAccountData = $this->proxy->GetPastSubscription($data);
            $this->request_status = $oAccountData->GetPastSubscriptionResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetPastSubscriptionResult);
            return $oAccountData;
            ...
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetPastSubscription',$eData)
            );
        }
    }
    public function UnsubscribeConnection($account_info) {
        $eData = array();
    ...
    public function UpdateCopySettings($account_info) {
        $eData = array();
        ...
        $data = array(
            'req' => array(
                'ConnectionId' => $account_info['connectionId'],
                'Definitions' => $copysettings['CopySettings'],
                'Id'          => $this->fm_service_id,
                'Password'    => 'eJR5RdZfA86s4m78',
                'Session'     => 'abc123456',
                'UserId'      => $account_info['accountNumber'],
    ...
    public function UpdateTradersCondition($account_info = array()){
        $eData = array();
        try {

            $data = array(
                'req' => array(
                    'Definitions'   => array( // accept 1 commission condition type only
                        '0'  => array(
                            'Key'   => $account_info['commission_type'],
                            'Value' => $account_info['commission_value'],
                        ),
                    ...
                    'Password'    => 'eJR5RdZfA86s4m78',
                    'Session'     => 'abc123456',
                    'UserId'      => $account_info['trader'],
                )
            );

            if(!empty($account_info['conditions_values_5'])){
                $data['req']['Definitions'][2]['Key'] = 5;
                $data['req']['Definitions'][2]['Value'] = $account_info['conditions_values_5'];
            }
...
//            }
            ...
            $this->request_status = $oAccountData->UpdateTradersConditionResult->ReqResult;
//            if(IPLoc::IPOnlyForVenus()){
//                var_dump($this->request_status); exit();
//            }
            $this->result = self::get_array_object($oAccountData->UpdateTradersConditionResult);
        } catch (SoapFault $e){
            return array(
    ...
    public function GetTicketInfo($account_info = array()){
        $eData = array();
        try {
            ...
            $oAccountData = $this->proxy->GetTicketInfo($data);
       
            $this->request_status = $oAccountData->GetTicketInfoResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->GetTicketInfoResult);
        } catch (SoapFault $e){
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetTicketInfo',$eData)
            );
        }
    }
    public function PayRollOver($account_info = array()){
        $eData = array();
        try {
            ...
            $oAccountData = $this->proxy->PayRollOver($data);

            $this->request_status = $oAccountData->PayRollOverResult->ReqResult;
            $this->result = self::get_array_object($oAccountData->PayRollOverResult);
        } catch (SoapFault $e){
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'PayRollOver',$eData)
            );
        }
    }

    public function RegisterForexCopy($account_info = array()){
        $eData = array();
        ...
        try {
            ...
            if($account_info['IsTrader']){
                $data = array(
                    'req' => array(
                        'Definitions'   => array( // accept 1 condition type only
                            '0'  => array(
                                'Key'   => $account_info['Commission_type'],
                                'Value' => $account_info['Commission_value'],
                            ),
    ...
    public function ActivateTrader($account_info = array()){
        $eData = array();
        ...
        try {
                ...
                $data = array(
                    'req' => array(
                        'UserId'      =>  $account_info['accountNumber'], // client login
                        'Id'          => $this->fm_service_id,
                        'Password'    => 'eJR5RdZfA86s4m78',
                        'Session'     => 'abc123456',
                    )
    ...
    public function DeactivateTrader($account_info = array()){
        $eData = array();
    ...
    public function GetAccountType($account_info = array()){
        $eData = array();
        ...
        try {
            ...
            $oAccountData = $this->proxy->GetAccountType($data);
            $this->request_status = $oAccountData->GetAccountTypeResult->Result;
            $this->result = self::get_array_object($oAccountData->GetAccountTypeResult);
        } catch (SoapFault $e){
            return array(
                'SOAPError' => true,
                'Message' => $e->getMessage(),
                'LogId' => self::Exception($e,'GetAccountType',$eData)
            );
        }
    }
    public function GetConnectionId($account_info = array()){
        $eData = array();
    ...
    public function UpdatePendingFollower($account_info = array()){
        $eData = array();
        ...
        try {
                ...
                $data = array(
                    'req' => array(
                        'ConnectionId' =>  $account_info['connection'],
                        'Status'      =>  $account_info['status'],
                        'UserId'      =>  $account_info['accountNumber'], // client login
                        'Id'          => $this->fm_service_id,
                        'Password'    => 'eJR5RdZfA86s4m78',
    ...
    public function UpdateTradersDescription($account_info = array()){
        $eData = array();
        try {
    ...
    public function SubscribeToTrader($account_info = array()){
        $eData = array();
        try {
            ...
            $data = array(
                'req' => array(
                    ...
                    'Trader'      => $account_info['trader'],
                    'UserId'      =>  $account_info['follower'], // client login
                    'Id'          => $this->fm_service_id,
                    'Password'    => 'eJR5RdZfA86s4m78',
                    'Session'     => 'abc123456',
                    'Definitions'   => array(
    ...
    public function UpdateTradersUserDetails($account_info = array()){
        $eData = array();
        try {
    ...
    public function request_inactive_account_details($data = array()){
